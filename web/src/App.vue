<template>
	<div id="app"
		...
		<!--尾部-->
		<CarverFooter v-if="show" :init="init"></CarverFooter>
		...
		<!--点击特效-->
		<div class="floatig">
			<font v-for="vo,i in keys.list" v-show="vo.show" :key="i" :style="{top:vo.yy+'px',left:vo.xx+'px',opacity:vo.opacity}">{{vo.name}}</font>
		</div>
	...
	export default {
		data() {
			return {
				...
				keys:{
					list:[
						{name:"有意思",show:false,yy:0,xx:0,opacity:1},
						{name:"更有意思",show:false,yy:0,xx:0,opacity:1},
						{name:"非常有意思",show:false,yy:0,xx:0,opacity:1},
						{name:"点个赞呗",show:false,yy:0,xx:0,opacity:1},
						{name:"打个赏呗",show:false,yy:0,xx:0,opacity:1}
					],
					i:0
				},
                isMobile:false
